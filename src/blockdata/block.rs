...
pub struct BlockHeaderWithoutAuxPow {
...
impl_consensus_encoding!(BlockHeaderWithoutAuxPow, version, prev_blockhash, merkle_root, time, bits, nonce);
...
pub struct AuxPow {
    ...
    pub parent_block_hash: BlockHeaderWithoutAuxPow
}

impl_consensus_encoding!(AuxPow, coinbase_tx, block_hash, coinbase_branch, n_index, blockchain_branch, chain_index, parent_block_hash);
...
impl Decodable for BlockHeader {
    fn consensus_decode<D: std::io::Read>(d: D) -> Result<Self, encode::Error> {
        ...
        let version: i32 = Decodable::consensus_decode(&mut d)?;
        let prev_blockhash: BlockHash = Decodable::consensus_decode(&mut d)?;
        let merkle_root: TxMerkleNode = Decodable::consensus_decode(&mut d)?;
        let time: u32 = Decodable::consensus_decode(&mut d)?;
        let bits: u32 = Decodable::consensus_decode(&mut d)?;
        let nonce: u32 = Decodable::consensus_decode(&mut d)?;

        let auxpow = if version & (1 << 8) != 0 {
            Some(AuxPow::consensus_decode(d)?)
        } else {
            None
        };
    ...
    fn consensus_decode_from_finite_reader<D: std::io::Read>(
        ...
        let version: i32 = Decodable::consensus_decode_from_finite_reader(&mut d)?;
        let prev_blockhash: BlockHash = Decodable::consensus_decode_from_finite_reader(&mut d)?;
        let merkle_root: TxMerkleNode = Decodable::consensus_decode_from_finite_reader(&mut d)?;
        let time: u32 = Decodable::consensus_decode_from_finite_reader(&mut d)?;
        let bits: u32 = Decodable::consensus_decode_from_finite_reader(&mut d)?;
        let nonce: u32 = Decodable::consensus_decode_from_finite_reader(&mut d)?;
        
        let auxpow = if version & (1 << 8) != 0 {
            Some(AuxPow::consensus_decode_from_finite_reader(&mut d)?)
        } else {
            None
        };
...
impl Encodable for BlockHeader {
    fn consensus_encode<S: std::io::Write>(&self, mut s: S) -> Result<usize, std::io::Error> {
       ...
       len += self.auxpow.as_ref().map(|x| x.consensus_encode(&mut s).ok()).flatten().unwrap_or_default();
...
impl BlockHeader {
    ...
    pub fn block_hash(&self) -> BlockHash {
        ...
        self.consensus_encode_without_auxpow(&mut engine).expect("engines don't error");
        BlockHash::from_engine(engine)
    }

    fn consensus_encode_without_auxpow<S: std::io::Write>(&self, mut s: S) -> Result<usize, std::io::Error> {
       let mut len = 0;
       len += self.version.consensus_encode(&mut s)?;
       len += self.prev_blockhash.consensus_encode(&mut s)?;
       len += self.merkle_root.consensus_encode(&mut s)?;
